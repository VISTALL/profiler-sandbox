...
import net.sf.profiler4j.agent.SnapshotConstants;
...
public class Snapshot
{
	...
	public static Snapshot read(InputStream is) throws IOException
	{
		...
		if(dis.readInt() != SnapshotConstants.SNAPSHOT_MAGIC)
		{
			throw new IOException("Invalid magic number");
		}
		if(dis.readInt() != SnapshotConstants.SNAPSHOT_PROTOCOL_VERSION)
		{
			throw new IOException("Unsupported protocol version");
		}
		if(dis.readInt() != SnapshotConstants.SNAPSHOT_TYPE_CALLTRACE)
		{
		...
		for(Method m : snapshot.methods.values())
		{
			...
			{
				m.childrenTimes.put(snapshot.methods.get(m.tmp_childrenIds[i]), m.tmp_childrenTimes[i] / 1e6);
			}
